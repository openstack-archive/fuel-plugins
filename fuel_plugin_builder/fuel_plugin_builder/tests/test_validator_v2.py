...
class TestValidatorV2(BaseTestCase):
    ...
    @mock.patch.object(ValidatorV2, 'check_compability')
    def test_validate(
            self,
            check_tasks_mock,
            check_schemas_mock,
            check_paths_mock,
            check_compability_mock):
        ...
        check_compability_mock.assert_called_once_with()
    ...
    def test_check_compability(self, parse_yaml_mock):
        parse_yaml_mock.return_value = {'fuel_version': ['6.0', '6.1']}

        with self.assertRaisesRegexp(
                errors.ValidationError,
                'This scheme can be used only for releases 6.1 or higher.'
                ' Fuel version 6.0 does not support. Please remove it or'
                ' downgrade package version'):
            self.validator.check_compability()
