...
from fuel_plugin_builder.tests.test_validator import BaseValidator
...
class TestValidatorV3(BaseValidator):
    ...
    def test_check_tasks_schema_validation_failed(
            self, exists_mock, utils_mock):
        data_sets = [
            {
                'type': 'shell',
                'parameters': {
    ...
    @mock.patch('fuel_plugin_builder.validators.base.utils')
    @mock.patch('fuel_plugin_builder.validators.base.os.path.exists')
    def test_check_tasks_schema_validation_passed(
            self, exists_mock, utils_mock):
        data_sets = [
            [
                {
