...
class ReportNode(object):
    ...
    def error(self, msg_or_exc, *args, **kwargs):
        ...
        if isinstance(msg_or_exc, six.string_types):
            self.add_nodes(
                ReportNode(error_prefix + u'{}'.format(msg_or_exc))
            )

        elif isinstance(msg_or_exc, Exception):
            self.add_nodes(
                ReportNode(error_prefix + u'{}'.format(msg_or_exc.message))
            )
        ...
        self.add_nodes(
            *(
                ReportNode(error_prefix + u'{}'.format(arg))
                for arg in args
            )
        )
        self.add_nodes(
            *(
                ReportNode(error_prefix + u'{}: {}'.format(key, kwargs[key]))
                for key in kwargs
            )
        )
        return self
