...
class SchemaTaskV2_0_0(SchemaTaskV1_0_0):
    ...
    def _gen_task_schema(self, task_types, required=None, parameters=None):
        ...
        return {
            ...
            'type': 'object',
            'required': list(set(
                ['id', 'type'] if not required else required
            )),
            'properties': {
                'type': {
                    'enum': task_types},
...
class SchemaTaskV2_1_0(object):
    ...
    _stage_pattern = '^(post_deployment|pre_deployment)' \
                     '(/[-+]?([0-9]*\\?.[0-9]|[0-9])+)?$'
    _role_aliases = ('roles', 'groups', 'role')
    task_name_pattern = '^[0-9a-zA-Z_-]+$|^\*$'
    _task_obligatory_fields = ('id', 'type')
    _task_role_pattern = '^[0-9a-zA-Z_-]+$|^\*$'
    _task_version_pattern = '^\d+.\d+.\d+$'
    ...
    def _gen_task_schema(self, task_types, required=None, parameters=None):
        ...
        task_specific_req_fields = set(
            self._task_obligatory_fields +
            (tuple(required) if required else ())
        )
        return {
            '$schema': 'http://json-schema.org/draft-04/schema#',
            'type': 'object',
...
class SchemaTaskV2_2_0(SchemaTaskV2_1_0):
    @property
    def tasks(self):
        return {
            '$schema': 'http://json-schema.org/draft-04/schema#',
    ...
    @property
    def move_to_bootstrap_task(self):
        return self._gen_task_schema(
            "move_to_bootstrap"
        )

    @property
    def master_shell_task(self):
        return self._gen_task_schema(
            "master_shell"
        )

    @property
    def erase_node_task(self):
        return self._gen_task_schema(
            "erase_node"
        )
