...
class LegacyBaseValidator(object):
    ...
    def validate_file_by_schema(self, schema, file_path,
        ...
        data = utils.parse_yaml_file(file_path)
        if data is not None:
    ...
    def check_env_config_attrs(self):
        ...
        structure.
        """
        config = utils.parse_yaml_file(self.env_conf_path)
        if not config:
            return
    ...
    def check_releases_paths(self):
        meta = utils.parse_yaml_file(self.meta_path)
...
class Inspection(object):
    def __init__(self, checks, name="", path="."):
        ...
        if checks:
            ...
            raise Exception("No checks provided")
        self.name = name
        self.path = path
    ...
    def check(self):
        ...
        given path.
        """
        report = ReportNode(text="Inspection: " + self.name)

        if not self.checks:
            raise Exception("No checks defined")

        # combine all messages related to path
        messages_by_path = {}

        for check in self.checks:
            # check path
            check_result = check.check_path(self.path)
            report_node = messages_by_path.setdefault(
                self.path, ReportNode(self.path))
            report_node.add_nodes(check_result)
            # check every file in path
            for file_path in utils.files_in_path(self.path):
                check_result = check.check_file(file_path)
                report_node = messages_by_path.setdefault(
                    file_path, ReportNode(file_path))
                report_node.add_nodes(check_result)

        # merge result to report tree
        for path, path_reports in six.iteritems(messages_by_path):
            report.add_nodes(path_reports)
...
class ReportNode(object):
    ...
    def add_nodes(self, nodes):
        ...
        :type nodes: list|dict|ReportNode
        """
        # single node
