...
class SchemaTaskV2_1(object):

    file_permissions_pattern = '^[0-7]{4}$'

    role_aliases = ('roles', 'groups', 'role')
    stage_pattern = '^(post_deployment|pre_deployment)(/[-+]?([0-9]*\.[0-9]+|[0-9]+))?$'
    ...
    def _gen_task_schema(self, task_types, required=None,
        ...
        task_specific_req_fields = list(set(
            self.task_obligatory_fields + (required or [])
        ))
        return {
            '$schema': 'http://json-schema.org/draft-04/schema#',
            'type': 'object',
            ...
            'properties': {
                ...
                'id': {
                    ...
                    'pattern': self.task_name_pattern},
                'version': {
                    'type': 'string', "pattern": self.task_version_pattern},
