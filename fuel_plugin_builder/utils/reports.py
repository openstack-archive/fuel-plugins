...
from utils.data_structures import Enum

# Size of the new level text indent when rendering report
TEXT_REPORT_INDENT_SIZE = 4

# Symbol to mark error nodes when rendering report
TEXT_REPORT_FAILURE_POINTER = '> '

# delimiter between lines of text reporter
TEXT_REPORT_LINE_DELIMITER = '\n'
...
class ReportNode(object):
    ...
    def _renderers(self):
        return {
            RENDER_FORMATS.text: self._render_text,
            RENDER_FORMATS.yaml: self._render_yaml,
            RENDER_FORMATS.json: self._render_json
        }
    ...
    def __init__(
        ...
        :type failed: boolean
        """
        self.text = self._process_incoming_message(text)
        self.time = time or datetime.datetime.now()
        self.children = children if children is not None else []
        self.level = level or None
        if self.level == REPORT_LEVELS.error:
            self.failed = True
    ...
    def _render_text(
        self,
        depth=None,
        indent_size=TEXT_REPORT_INDENT_SIZE,
        failure_pointer=TEXT_REPORT_FAILURE_POINTER,
        line_delimiter=TEXT_REPORT_LINE_DELIMITER
    ):

        indent = indent_size * (depth or 0) * ' '
        error_indent_size = max(len(indent) - len(failure_pointer), 0)
        error_indent = error_indent_size * ' '

        lines = []
        failed = self.failed
        # if self.text is not None or self.level is not None:
        #     next_level = (depth or 0) + 1
        # else:
        #     next_level = depth
        for child in self.children:
            child_lines = child._render_text(
                ...
                failure_pointer=failure_pointer,
                line_delimiter=line_delimiter
            )
            # failed = child.is_failed() or failed
        ...
        if self.text or self.level:
            output = '{indent}{pointer}{text}'.format(
                ...
                pointer=failure_pointer if failed else '',
    ...
    def _process_incoming_message(self, msg_or_exc):
        if not msg_or_exc:
            return msg_or_exc
        if isinstance(msg_or_exc, six.string_types):
            ...
            if tb:
                self.info(tb)
            return msg_or_exc.message or repr(msg_or_exc)
        else:
            return "{}".format(msg_or_exc)

    def _make_message(self, msg_or_exc, level, *args, **kwargs):

        self.add_nodes(
            ReportNode(self._process_incoming_message(msg_or_exc), level)
        )
        self.add_nodes(
            *(
