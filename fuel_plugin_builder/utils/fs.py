...
def get_files_by_mask(path_mask, allowed_formats=None):
    """Find all files of allowed format in path.

    :param path_mask: path mask like ./my-file.*
    :type path_mask: str|basestring

    :param allowed_formats: available file formats, allow all if not defined
    ...
    raise errors.FilesInPathDoesNotExist(
        u"Can't find file. "
        u"Ensure that file is on its place and have one of the following data "
        u"files formats: {}.".format(
            u", ".join(list(allowed_formats))
        )
    )


def get_best_file_by_mask(path_mask, allowed_formats):
    ...
    paths = get_files_by_mask(path_mask, allowed_formats=allowed_formats)
    if paths:
        return paths[0]
    else:
        ...
        raise errors.FilesInPathDoesNotExist(
            u"Can't find file. "
            u"Ensure that file is on its place and have one of the "
            u"following data files formats: {}.".format(
                u", ".join(list(allowed_formats))
            )
        )
