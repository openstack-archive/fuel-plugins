...
from builders import BuilderV1
from builders import BuilderV2
from builders import BuilderV3
...
class BaseBuildTestCase(BaseTestCase):
    ...
    def _create_builder(self, plugin_path):
        self.builder_class.loader_class.load = mock.Mock(
            return_value=(self.fake_metadata, utils.ReportNode())
        )
    ...
    @mock.patch('utils.which')
    @mock.patch('utils.exec_cmd',
                return_value=True)
    ...
    @mock.patch('utils.create_dir')
    @mock.patch('utils.copy_files_in_dir')
    ...
    @mock.patch('utils.exec_piped_cmds')
    @mock.patch('utils.load_template_and_render_to_file')
    ...
    @mock.patch('utils.create_dir')
    @mock.patch('utils.move_files_in_dir')
    @mock.patch('utils.exec_cmd')
    ...
    @mock.patch('version_mapping.get_validator')
    ...
    @mock.patch('utils.remove')
    @mock.patch('utils.create_dir')
    @mock.patch('utils.remove_by_mask')
...
class TestBuilderV2(BaseBuildTestCase):
    ...
    @mock.patch('utils.get_current_year')
    @mock.patch('utils.create_dir')
    @mock.patch('utils.make_tar_gz')
    @mock.patch('utils.load_template_and_render_to_file')
    @mock.patch('utils.exec_cmd')
...
class TestBuilderV3(BaseBuildTestCase):
    ...
    @mock.patch('utils.exec_cmd')
    @mock.patch('utils.copy_files_in_dir')
    @mock.patch('utils.read_if_exist')
    def _test_make_package(self, builder, plugin_path, read_if_exist_m,
                           copy_files_in_dir_m, exec_cmd_m,
                           make_tar_gz_m,
                           create_dir_m, get_current_year_m):
