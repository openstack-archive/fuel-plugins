...
class BaseLoaderTestCase(FakeFSTest):
    plugin_path = os.path.abspath('../templates/v5/plugin_data')
    loader = loaders.BaseLoader()
    validator = validators.BaseValidator()

    def setUp(self):
        super(BaseLoaderTestCase, self).setUp()
        self.data_tree = self.loader.load(self.plugin_path)
...
class LoaderV5TestCase(BaseLoaderTestCase):

    plugin_path = os.path.abspath('../templates/v5/plugin_data')
    validator = validators.ValidatorV5()
    loader = loaders.LoaderV5()
    ...
    def test_loaded_ok(self):
        ...
        self.assertEqual(RELEASE_V5_DATA, data)
