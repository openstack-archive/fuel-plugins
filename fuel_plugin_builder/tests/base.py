...
class FakeFSTest(BaseTestCase, fake_filesystem_unittest.TestCase):
    ...
    def _create_fakefs_yaml(self, path, new_data):
        """Replace file with new one inside file system mock.manag
    ...
    def setUp(self):
        ...
        print("Setting up fakeFs for {}".format(self.plugin_path))
        if self.plugin_path:  # load FakeFS
            for root, _, file_names in os.walk(self.plugin_path):
                for filename in file_names:
                    ...
                    if extension == consts.TEMPLATE_EXTENSION:
                        ...
                        dst_path = utils.fs.get_path_without_extension(
                            src_path)
                    else:
                        dst_path = src_path
