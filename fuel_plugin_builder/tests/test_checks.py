...
from fuel_plugin_builder.validators.checks import YamlLoadMixin
...
class TestChecks(BaseTestCase):
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.exists')
    @mock.patch('fuel_plugin_builder.validators.checks.utils.isfile')
    def test_multi_json_schema_is_valid(
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_is_compatible_ok(self, parse_yaml_m):
        fuel_version_checks = (
            (['6.0', '6.1', '7.0', '8.0']),
            (['6.1', '7.0', '8.0']),
        )
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_is_compatible_fail(self, parse_yaml_m):
        fuel_version_checks = (
            (['6.0', '6.1', '7.0', '8.0', '9.0']),
            (['6.1', '7.0', '8.0']),
        )
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.exists')
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_env_scripts_and_repo_paths_ok(self, parse_yaml_m, exists_m):
        parse_yaml_m.return_value = {
            'releases': [{
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_env_config_attr_pass_do_not_fail_if_empty(self, parse_yaml_m):
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_check_env_config_attrs_1_fail_if_none(self, parse_yaml_m):
        check = EnvConfigAttrsIsValidV1()
        parse_yaml_m.return_value = {'attributes': None}
        result = check.check_file('environment_config.yaml')
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_check_env_config_attrs_1_checks_metadata_fail(self, parse_yaml_m):
        check = EnvConfigAttrsIsValidV1()
        parse_yaml_m.return_value = {'attributes': {'metadata': []}}
        result = check.check_file('environment_config.yaml')
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_check_env_config_attrs_1_checks_attrs_fail(self, parse_yaml_m):
        check = EnvConfigAttrsIsValidV1()
        parse_yaml_m.return_value = {
            'attributes': {
                'key1': {
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_check_env_config_attrs_1_restriction_fail(self, parse_yaml_m):
        check = EnvConfigAttrsIsValidV1()
        parse_yaml_m.return_value = {
            'attributes': {
                'key1': {
    ...
    @mock.patch('fuel_plugin_builder.validators.checks.utils.parse_yaml')
    def test_check_env_config_attrs_1_do_not_fail(self, parse_yaml_m):
        check = EnvConfigAttrsIsValidV1()
        parse_yaml_m.return_value = {
            'attributes': {
                'key1': {
    ...
    def test_yaml_load_mixin_fail_on_bad_file(self):
        mixin_instance = YamlLoadMixin()
        report_node = ReportNode()
        mixin_instance._load_yaml('/verybadF1le', report_node)
        self.assertEqual(1, report_node.count_failures())
        self.assertIn(
            "ERROR: I/O error(2): No such file or directory",
            report_node.render())

    @mock.patch('__builtin__.open')
    def test_yaml_load_mixin(self, open_m):
        mixin_instance = YamlLoadMixin()
        report_node = ReportNode()
        bad_yaml = "\n:\n-"
        open_m.return_value = six.moves.StringIO(bad_yaml)
        mixin_instance._load_yaml('file.yaml', report_node)
        self.assertIn(
            "ERROR: Can't parse YAML file, error position: (2:1)",
            report_node.render())
