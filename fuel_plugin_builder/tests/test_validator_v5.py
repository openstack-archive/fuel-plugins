...
class BaseValidatorTestCase(FakeFSTest):
    ...
    ValidatorClass = validators.BaseValidator
    LoaderClass = loaders.BaseLoader

    def setUp(self):
        super(BaseValidatorTestCase, self).setUp()
        self.validator = self.ValidatorClass()
        self.data_tree = self.LoaderClass().load(self.plugin_path)


class ValidatorV5TestCase(BaseValidatorTestCase):
    ValidatorClass = validators.ValidatorV5
    LoaderClass = loaders.LoaderV5

    def test_validate(self):
        report = self.validator.validate(self.data_tree)
        self.assertIn(u'Validation successful!', report.render())
