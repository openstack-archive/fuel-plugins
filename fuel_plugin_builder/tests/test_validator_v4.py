...
class TestValidatorV4(TestValidatorV3):
    ...
    def test_warn_if_required_is_missing(self, logger_m, utils_m, *args):
        ...
        for task in deployment_tasks_data:
            utils_m.parse_yaml.return_value = [task]
            self.validator.check_deployment_tasks()
            self.assertTrue(logger_m.warn.called)
    ...
    @mock.patch('fuel_plugin_builder.validators.validator_v4.utils')
    def test_check_task_role_regexp_failed(self, utils_mock, *args):
        mock_data = [{
            'id': 'plugin_name',
            'type': 'group',
            'role': '/[0-9]++/'}]
