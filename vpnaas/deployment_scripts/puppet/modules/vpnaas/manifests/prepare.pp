#This class modifies product manifests to deploy VPN agent instead of L3 agent in HA mode.
#L3 and VPN agents use the same configuration file and they are deployed in the same way.
#So as the easiest solution we can simply replace L3 binaries to VPN binaries and call the same classes.
#Also we do some preliminary work as L3 resource removal and VPNaaS agent basic deployment (conf files and binaries).
